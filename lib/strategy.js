...
Strategy.prototype.authenticate = function(req, options) {
  ...
  this._verify = function (req, username, password, company, apiKey, next) {
    var reqOptions = {
      uri: 'http://api.bamboohr.com/api/gateway.php/' + company + "/v1/login",
      form: {
        user: username,
        password: password,
    ...
    request(reqOptions, function(error, response, body) {
      if (error || !body.success) {
        return done(error || "Error: Could not authenticate with BambooHR.");
      } else {
        var user = {
          username: username,
          company: company,
  ...
  try {
    if (self._passReqToCallback) {
      ...
      this._verify(username, password, company, this._apiKey, this._next);
    }
  } catch (ex) {
    return self.error(ex);
  }
